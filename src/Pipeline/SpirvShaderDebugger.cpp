...
using uint2 = vec<uint_t, 2>;
using uint3 = vec<uint_t, 3>;
using uint4 = vec<uint_t, 4>;
...
struct PerLane
{
	vec<T, N> operator[](int lane) const
	{
		vec<T, N> out;
		for(int i = 0; i < N; i++)
		{
	...
	std::array<std::array<T, sw::SIMD::Width>, N> data;
};
...
struct PerLane<T, 1>
{
	...
	std::array<T, sw::SIMD::Width> data;
};

using uint_t = PerLane<math::uint_t>;
using uint2 = PerLane<math::uint_t, 2>;
using uint3 = PerLane<math::uint_t, 3>;
using uint4 = PerLane<math::uint_t, 4>;

using int_t = PerLane<math::int_t>;
using int2 = PerLane<math::int_t, 2>;
using int3 = PerLane<math::int_t, 3>;
using int4 = PerLane<math::int_t, 4>;

using float_t = PerLane<math::float_t>;
using vec2 = PerLane<math::float_t, 2>;
using vec3 = PerLane<math::float_t, 3>;
using vec4 = PerLane<math::float_t, 4>;
...
template<typename T, int N>
std::shared_ptr<vk::dbg::Value> makeDbgValue(const math::vec<T, N> &vec)
{
...
struct SpirvShader::Impl::Debugger : public vk::dbg::ClientEventListener
{
	...
	// Traps holds information about debugger traps.
	// A trap is a point in a program where it may pause for the debugger,
	// either due to a set breakpoint or a single line step.
	struct Traps
	{
	...
	// Shadow memory is used to construct a contiguous memory block for Object
	// values. Every SSA value has a shadow entry.
	struct Shadow
	{
		struct Entry
		{
...
public:
	...
	struct Globals
	{
		struct Compute
		{
			math::uint3 numWorkgroups;
			math::uint3 workgroupID;
			math::uint3 workgroupSize;
			math::uint_t numSubgroups;
			math::uint_t subgroupIndex;
			math::SIMD::uint3 globalInvocationId;
			math::SIMD::uint3 localInvocationId;
			math::SIMD::uint3 localInvocationIndex;
		};
		struct Fragment
		{
			math::uint_t viewIndex;
			math::SIMD::vec4 fragCoord;
			math::SIMD::vec4 pointCoord;
			math::SIMD::int2 windowSpacePosition;
			math::SIMD::uint_t helperInvocation;
		};
		struct Vertex
		{
			math::uint_t viewIndex;
			math::uint_t instanceIndex;
			math::SIMD::uint_t vertexIndex;
		};

		math::uint_t subgroupSize;
		math::SIMD::uint_t activeLaneMask;
		union
		{
			Compute compute;
			Fragment fragment;
	...
	private:
		...
		void updateFrameVariables(State *state, vk::dbg::Frame &frame, debug::LexicalBlock *block);
		...
		template<typename T, int N>
		void buildGlobal(const char *name, const math::SIMD::PerLane<T, N> &vec);
		void buildGlobals(State *state);
		std::shared_ptr<vk::dbg::Value> buildSpirvVariables(State *state, int lane) const;
...
{
	// TODO: Lock around traps
	// Clear all traps.
	memset(traps.memory.get(), 0, traps.byIndex.size() * sizeof(uint8_t));

	// Add traps for all breakpoints.
	for(auto it : files)
	{
		auto &file = it.second;
...
////////////////////////////////////////////////////////////////////////////////
// SpirvShader::Impl::Debugger::Shadow
////////////////////////////////////////////////////////////////////////////////
...
SpirvShader::Impl::Debugger::Shadow::Memory::dref(int lane) const
{
	...
	{
		uint8_t *base;
		uint32_t offsets[sw::SIMD::Width];
	};
...
void sw::SpirvShader::Impl::Debugger::LocalVariableValue::updateValue()
{
	...
	{
		ASSERT(activeValue->local == shared->variable);
...
{
	if(!data) { data = std::make_unique<Data>(this); }
	...
	if(locationAndScope.scope)
	{
			...
			if(stack[i] != newStack[i])
			{
				...
				if(isTopMostFrame && oldFunction == newFunction)
				{
					...
					thread->update(true, [&](vk::dbg::Frame &frame) {
						updateFrameVariables(state, frame, stack[i].block);
					});
				}
				else
				{
		...
		for(size_t i = stack.size(); i < newStack.size(); i++)
		{
			...
			thread->enter(entry.block->source->dbgFile, function->name, [&](vk::dbg::Frame &frame) {
				...
				updateFrameVariables(state, frame, entry.block);
			});
		}
	}
...
void SpirvShader::Impl::Debugger::State::Data::updateFrameVariables(State *state, vk::dbg::Frame &frame, debug::LexicalBlock *block)
{
...
SpirvShader::Impl::Debugger::State::PerLaneVariables
...
template<typename T, int N>
void SpirvShader::Impl::Debugger::State::Data::buildGlobal(const char *name, const math::SIMD::PerLane<T, N> &simd)
{
	for(int lane = 0; lane < sw::SIMD::Width; lane++)
	{
		for(int i = 0; i < N; i++)
...
void SpirvShader::Impl::Debugger::State::Data::buildGlobals(State *state)
{
	...
	for(int lane = 0; lane < sw::SIMD::Width; lane++)
	{
		...
		vc->put("SPIR-V", buildSpirvVariables(state, lane));
