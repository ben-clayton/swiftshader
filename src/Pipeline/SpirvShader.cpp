...
namespace
{
	...
	public:
		...
		public:
			...
			Group group(RK key) { return Group(ctx, rr::Call(S::group<K>, ctx, ptr, key)); }

			template<typename K, typename V, typename RK, typename RV>
			void put(RK key, RV value) { rr::Call(S::put<K, V>, ctx, ptr, key, value); }
	...
	private:
		...
		struct S {
			...
			static vk::dbg::VariableContainer* group(Ctx* ctx, vk::dbg::VariableContainer* vc, K key) {
				return ctx->group(vc, key);
			}

			template<typename K, typename V>
			static void put(Ctx* ctx, vk::dbg::VariableContainer* vc, K key, V value) {
				ctx->put(vc, key, value);
			}
		};
	};
	// << DEBUGSERVER
...
namespace rr {
	...
	template<>
	struct CToReactor<const std::shared_ptr<vk::dbg::File>*>
	{
		using type = rr::Pointer<Byte>;
		static type cast(const std::shared_ptr<vk::dbg::File>* v) { return rr::ConstantPointer(v); }
	};
} // namespace rr
...
namespace sw
{
				  ...
				  robustBufferAccess(robustBufferAccess)
	{
		...
		for (auto insn : *this)
		{
			...
			case spv::OpModuleProcessed:
				...
				break;
			case spv::OpName:
				names.emplace(insn.word(1), insn.string(2));
				break;
			case spv::OpString:
	...
	SpirvShader::EmitResult SpirvShader::EmitInstruction(InsnIterator insn, EmitState *state) const
	{
		...
		case spv::OpLine:
		{
			...
			dbgLine(path, line, column, state);
			return EmitResult::Continue;
		}
		...
		case spv::OpSpecConstantComposite:
		case spv::OpSpecConstantOp:
			dbgExposeIntermediate(insn.word(2), state);
			return EmitResult::Continue;
	...
	SpirvShader::EmitResult SpirvShader::EmitVariable(InsnIterator insn, EmitState *state) const
	{
		...
		dbgExposeIntermediate(resultId, state);
	...
	SpirvShader::EmitResult SpirvShader::EmitLoad(InsnIterator insn, EmitState *state) const
	{
		...
		defer(dbgExposeIntermediate(resultId, state));
