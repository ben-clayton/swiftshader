...
#include "GDP/dap/protocol.h"
#include "GDP/dap/server.h"
...
class WeakMap {
 public:
  ...
  std::shared_ptr<V> add(const K& key, ARGS&&... args);
 ...
 private:
  ...
  std::unordered_set<K> pendingRemove;
};

template <typename K, typename V>
void WeakMap<K, V>::Deleter::operator()(V* ptr) {
  map->remove(key);
  delete ptr;
}
...
template <typename K, typename V>
template <typename F>
void WeakMap<K, V>::foreach (const F& f) {
  lock();
  for (auto it : map) {
    if (auto sptr = it.second.lock()) {
      f(it.first, std::shared_ptr<V>(it.second));
    }
  }
  unlock();
}
...
template <typename K, typename V>
std::shared_ptr<V> WeakMap<K, V>::get(const K& key) {
  lock();
...
std::shared_ptr<V> WeakMap<K, V>::add(const K& key, ARGS&&... args) {
  auto sptr = std::shared_ptr<V>(new T(std::forward<ARGS>(args)...),
                                 Deleter{this, key});
  lock();
  map.emplace(key, sptr);
  {
    std::unique_lock<std::mutex> pendinglock(pendingMutex);
    pendingRemove.erase(key);
  }
  unlock();
  return sptr;
}
...
template <typename K, typename V>
void WeakMap<K, V>::lock() {
  return mapMutex.lock();
}

template <typename K, typename V>
bool WeakMap<K, V>::trylock() {
  return mapMutex.try_lock();
}

template <typename K, typename V>
void WeakMap<K, V>::unlock() {
  {
    std::unique_lock<std::mutex> pendinglock(pendingMutex);
    for (auto k : pendingRemove) {
      map.erase(k);
    }
    pendingRemove.clear();
  }
...
class Server::Impl : public Server {
 ...
 private:
  ...
  std::unique_ptr<dap::Server> server;
  Files files;
...
Server::Impl::Impl() : server(dap::Server::create()) {
  server->registerRequestHandler<dap::DisconnectRequest>(
      [](dap::Connection* conn, const dap::DisconnectRequest* req) {
        printf("DisconnectRequest receieved\n");
        dap::DisconnectResponse response;
        return response;
      });

  server->registerRequestHandler<dap::InitializeRequest>(
      [&](dap::Connection* conn, const dap::InitializeRequest* req) {
  ...
  server->registerResponseSentHandler<dap::InitializeResponse>(
      [](dap::Connection* conn,
         const dap::ResponseOrError<dap::InitializeResponse>& response) {
        printf("InitializeResponse sent\n");
        conn->send(dap::InitializedEvent());
      });

  server->registerRequestHandler<dap::SetExceptionBreakpointsRequest>(
      [](dap::Connection* conn,
  ...
  server->registerRequestHandler<dap::SetFunctionBreakpointsRequest>(
      [this](dap::Connection* conn,
  ...
  server->registerRequestHandler<dap::SetBreakpointsRequest>(
      [this](dap::Connection* conn, const dap::SetBreakpointsRequest* req)
  ...
  server->registerRequestHandler<dap::ThreadsRequest>(
      [this](dap::Connection* conn, const dap::ThreadsRequest* req) {
        printf("ThreadsRequest receieved\n");
        std::unique_lock<std::recursive_mutex> lock(mutex);
        dap::ThreadsResponse response;
        threads.foreach (
            [&](Thread::Id, const std::shared_ptr<Thread>& thread) {
              dap::Thread out;
              out.id = thread->id;
  ...
  server->registerRequestHandler<dap::StackTraceRequest>(
      [this](dap::Connection* conn, const dap::StackTraceRequest* req)
  ...
  server->registerRequestHandler<dap::ScopesRequest>(
      [this](dap::Connection* conn, const dap::ScopesRequest* req)
  ...
  server->registerRequestHandler<dap::VariablesRequest>(
      [this](dap::Connection* conn, const dap::VariablesRequest* req)
        ...
        vars->foreach (req->start.value(0), [&](const Variable& v) {
          if (!req->count.has_value() ||
            ...
            if (v.value->type()->kind == Kind::VariableContainer) {
              auto const vc =
                  static_cast<const VariableContainer*>(v.value.get());
  ...
  server->registerRequestHandler<dap::SourceRequest>(
      [this](dap::Connection* conn, const dap::SourceRequest* req)
  ...
  server->registerRequestHandler<dap::PauseRequest>(
      [this](dap::Connection* conn, const dap::PauseRequest* req)
        ...
        if (auto thread = threads.get(req->threadId)) {
          ...
          threads.foreach ([&](Thread::Id id, const std::shared_ptr<Thread>&) {
            event.threadId = id;
          });
        }
  ...
  server->registerRequestHandler<dap::ContinueRequest>(
      [this](dap::Connection* conn, const dap::ContinueRequest* req)
  ...
  server->registerRequestHandler<dap::NextRequest>(
      [this](dap::Connection* conn, const dap::NextRequest* req)
  ...
  server->registerRequestHandler<dap::StepInRequest>(
      [this](dap::Connection* conn, const dap::StepInRequest* req)
  ...
  server->registerRequestHandler<dap::StepOutRequest>(
      [this](dap::Connection* conn, const dap::StepOutRequest* req)
  ...
  server->registerRequestHandler<dap::EvaluateRequest>(
      [this](dap::Connection* conn, const dap::EvaluateRequest* req)
  ...
  server->registerRequestHandler<dap::LaunchRequest>(
      [](dap::Connection* conn, const dap::LaunchRequest* req) {
        printf("LaunchRequest receieved\n");
        return dap::LaunchResponse();
      });

  marl::WaitGroup configurationDone(1);
  server->registerRequestHandler<dap::ConfigurationDoneRequest>(
      [=](dap::Connection* conn, const dap::ConfigurationDoneRequest* req) {
...
std::shared_ptr<Thread> Server::Impl::currentThread() {
  ...
  auto thread = threads.add(id, id, this);
  thread->setName(name);
...
std::shared_ptr<File> Server::Impl::createVirtualFile(const char* name,
  ...
  if (clientIsVisualStudio) { // WORKAROUND: https://github.com/microsoft/VSDebugAdapterHost/issues/15
  ...
  return files.add(std::make_shared<VirtualFile>(id, sanitizedName.c_str(), source));
}
...
std::shared_ptr<VariableContainer> Server::Impl::createVariableContainer() {
  auto id = nextVariableContainerId++;
  return variableContainers.add(id, id);
}

std::shared_ptr<Scope> Server::Impl::createScope(
    const std::shared_ptr<File>& file) {
  auto id = nextScopeId++;
  return scopes.add(id, id, file, createVariableContainer());
}

std::shared_ptr<Frame> Server::Impl::createFrame(
    const std::shared_ptr<File>& file) {
  auto id = nextFrameId++;
  auto out = frames.add(id, id);
  out->arguments = createScope(file);
  out->locals = createScope(file);
  out->registers = createScope(file);
  return out;
}
...
Thread::Thread(int id, Server::Impl* server) : id(id), server(server) {}
