...
struct Type {
  inline Type() = default;
  inline Type(Kind kind) : kind(kind) {}
  Kind kind;
...
struct Constant : public Value {
 public:
  inline Constant(const T& value) : value(value) {}
...
struct VariableContainer : public Value {
  ...
  inline VariableContainer(Id id) : id(id) {}
  const Id id;
...
struct Scope {
  ...
  inline Scope(Id id,
      ...
      : id(id), file(file), variables(variables) {}
  const Id id;
  const std::shared_ptr<File> file;
  const std::shared_ptr<VariableContainer> variables;
};
...
class Thread {
 public:
  ...
  enum class State { Running, Stepping, Paused };
